...
def _make_guest_user(rules: list[dict]) -> GuestUser:
    token = {
        "user": {},
        "resources": [
...
def _guest_rls_filter(guest_user: GuestUser):
    ...
    def _filter(dataset: MagicMock) -> list[dict]:
        return [
            rule
...
def test_scoped_guest_rule_preserved_in_virtual_dataset(app: Flask) -> None:
    ...
    scoped_rule = {"dataset": str(pd_id), "clause": "tenant_id = 5"}
...
def _make_datasource_with_real_rls(dataset_id: int) -> MagicMock:
    ...
    datasource.get_sqla_row_level_filters = (
        lambda **kwargs: BaseDatasource.get_sqla_row_level_filters(
            datasource, **kwargs
        )
    )
    return datasource
...
def test_global_guest_rule_excluded_through_get_predicates_for_table(
    ...
    pd_id = 42
    global_rule = {"clause": "org_id = 1"}  # no "dataset" key → global

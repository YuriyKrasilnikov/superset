...
class TaskRestApi(BaseSupersetModelRestApi):
    ...
    def _resolve_export_artifact(
        task_uuid: UUID,
        user_id: int,
    ) -> ChartDataExportArtifact | None:
        """Resolve an owned, successful, unexpired, still-authorized artifact."""
        from superset.daos.tasks import TaskDAO

        task = TaskDAO.find_one_or_none(uuid=task_uuid)
        if task is None or task.status != TaskStatus.SUCCESS.value:
            return None
        if task.user_id != user_id and not security_manager.is_admin():
            return None
        ...
        datasource = DatasourceDAO.get_datasource(
            DatasourceType(artifact.datasource_type),
            artifact.datasource_id,
        )
    ...
    @expose("/<task_uuid>/artifact", methods=("GET",))
    @protect()
    @safe
    @statsd_metrics
    @event_logger.log_this_with_context(
        action=lambda self, *args, **kwargs: f"{self.__class__.__name__}.artifact",
        log_to_statsd=False,
    )
    def artifact(self, task_uuid: str) -> Response:
        """Download the unexpired artifact generated by a private task.
        ---
        get:
          summary: Download a completed private task artifact
          parameters:
          - in: path
            schema:
              type: string
          ...
          responses:
            200:
              description: Generated chart-data export
              content:
                text/csv:
                  schema:
                    type: string
        ...
        """
        user_id = get_user_id()
        if not is_feature_enabled("CHART_DATA_ASYNC_EXPORTS") or user_id is None:
            return self.response_404()
        if not self._can_export_data():
            return self.response_403()

        try:
            artifact = self._resolve_export_artifact(UUID(task_uuid), user_id)
            if artifact is None:
                return self.response_404()
            artifact_file = get_chart_data_artifact_store().open(artifact.storage_key)
        except (
            DatasourceNotFound,
            DatasourceTypeNotSupportedError,
            DatasourceValueIsIncorrect,
        ...
        response.headers["Content-Length"] = str(artifact.size_bytes)
        response.headers["ETag"] = f'"{artifact.sha256}"'
        response.call_on_close(artifact_file.close)
        return response

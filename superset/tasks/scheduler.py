...
@celery_app.task(name="prune_chart_data_export_artifacts", bind=True)
def prune_chart_data_export_artifacts(
    self: Task,
    max_rows_per_run: int | None = None,
    **kwargs: Any,
) -> None:
    """Remove expired chart-data export artifacts from storage and metadata."""
    from superset.commands.chart.data.prune_export_artifacts import (
        ChartDataExportArtifactPruneCommand,
    )

    stats_logger: BaseStatsLogger = current_app.config["STATS_LOGGER"]
    stats_logger.incr("prune_chart_data_export_artifacts")
    ChartDataExportArtifactPruneCommand(max_rows_per_run).run()
...
def execute_task(
    ...
    task = TaskDAO.find_one_or_none(uuid=UUID(task_uuid))
    if not task:
    ...
    final_task = execute_task_callable(
        ...
        args=args,
        kwargs=kwargs,
        timeout=task.properties_dict.get("timeout"),
    )
    return {"status": final_task.status, "task_uuid": task_uuid}
